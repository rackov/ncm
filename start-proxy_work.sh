...
# Запускаем контейнер с использованием томов вместо прямого монтирования файла
docker run -d \
...
# Копируем файл конфигурации в запущенный контейнер
docker cp ./config/setup.toml proxy-container:/app/config/setup.toml

# Перезапускаем контейнер, чтобы применить новую конфигурацию
